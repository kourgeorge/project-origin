...
class BrainPGTF:
    ...
    @staticmethod
    def init_session():
        if BrainPGTF.sess is None:
            tf.reset_default_graph()
            BrainPGTF.sess = tf.Session()
    ...
    def __init__(self, lr, s_size, action_size, h_size, scope, gamma, copy_from_scope=None):
        ...
        taken_action_probability = BrainPGTF.get_decision_probability(self.action_holder, self.action_distribution)
        ...
        BrainPGTF.sess.run(tf.variables_initializer(tf.get_collection(tf.GraphKeys.VARIABLES, scope)))

        self.saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.VARIABLES, scope))

        if copy_from_scope is not None:
            BrainPGTF.sess.run(utils.update_target_graph(copy_from_scope, scope))
    ...
    def save_model(self, path):
        self.saver.save(BrainPGTF.sess, path)

    def load_model(self, path):
        self.saver.restore(BrainPGTF.sess, path)

    def act(self, obs):
        action_dist = BrainPGTF.sess.run(self.action_distribution, feed_dict={self.state_in: [obs]})
    ...
    def train(self, batch_obs, batch_acts, batch_rews, batch_newstate):
        ...
        BrainPGTF.sess.run([self.optimize], feed_dict=feed_dict)

...
class Evolution:
    @staticmethod
    def random_dna():
        vsight = randint(0, round(Config.ConfigPhysics.SPACE_SIZE / 5))
        learning_rate = max(np.random.normal(loc=Config.ConfigBrain.LEARNING_RATE, scale=0.001), 1e-6)
        hidden_layer_size = Config.ConfigBrain.HIDDEN_LAYER_SIZE + randint(-31, 52)
    ...
    def mix_dna(dna1, dna2):
        ...
        hidden_layer_size = max(1, int(np.mean([dna1[2], dna2[2]])) + randint(-2, 2))

...
class Evolution:
    @staticmethod
    def random_dna():
        vision_range = 2  # randint(1, Config.ConfigBiology.BASE_VISION_RANGE)
        ...
        return [vision_range, learning_rate, hidden_layer_size, learning_frequency, max_age, gamma]

    @staticmethod
    def mix_dna(dna1, dna2):
        vision_range = 2  # max(1, int(np.mean([dna1[0], dna2[0]]) + randint(-1, 1)))

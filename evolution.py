...
class Evolution:
    ...
    def mix_dna(dna1, dna2):
        new_dna = DNA(np.mean([dna1.memory_size(), dna2.memory_size()]),
                      ...
                      np.mean([dna1.brain_structure_param(), dna2.brain_structure_param()]),
    ...
    def mutate_dna(dna):
        ...
        brain_structure_param = max(2, dna.brain_structure_param() + randint(-1, +1))
        learning_frequency = max(dna.learning_frequency() + randint(-1, 1), 1)
        life_expectancy = max(0, dna.life_expectancy() + randint(-10, 10))
        reward_discount = max(0.1, min(1, np.random.normal(loc=dna.reward_discount(), scale=0.1)))
        ...
        return DNA(memory_size, learning_rate, brain_structure_param, learning_frequency, life_expectancy, reward_discount, fitrah)


class DNA:
    def __init__(self, base_memory_size,
                 base_learning_rate,
                 base_brain_structure_param,
        ...
        self._brain_structure_param = base_brain_structure_param
    ...
    def brain_structure_param(self):
        return self._brain_structure_param
    ...
    def flatten(self):
        return [self.memory_size(), self.learning_rate(), self.learning_frequency(), self.brain_structure_param(),

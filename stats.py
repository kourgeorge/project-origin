...
class Stats:
    ...
    def collect_step_stats(universe):
        return OrderedDict([
            ...
            ('HLayer', np.round(np.mean([creature.brain_hidden_layer() for creature in universe.get_all_creatures()]), 2)),
            ('LFreq', np.round(np.mean([creature.learning_frequency() for creature in universe.get_all_creatures()]), 2)),
